...
const personalDb = config.DATABASE.define("personalDB", {
  ...
  lid_mapping: { type: DataTypes.TEXT, allowNull: true, defaultValue: "false" },
  welcome: { type: DataTypes.TEXT, allowNull: true, defaultValue: "{}" },
});

// Ensure DB table has any new columns the model expects (migrate if needed)
...
try {
  ...
  if (!tableInfo.lid_mapping) {
    await queryInterface.addColumn(personalDb.tableName, "lid_mapping", {
      type: DataTypes.TEXT,
      allowNull: true,
      defaultValue: "false",
    });
  }
  if (!tableInfo.welcome) {
    await queryInterface.addColumn(personalDb.tableName, "welcome", {
      type: DataTypes.TEXT,
      allowNull: true,
      defaultValue: "{}",
    });
  ...
  // if table doesn't exist yet or describeTable fails, ignore migration here
}*/
